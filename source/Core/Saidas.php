...
class Saidas {
    ...
    public function Total() {
        ...
        $read->ExeRead("app_faturas", "WHERE  status = :s  AND modo = :v AND carteira_id = :c",
                "s=paid&v=saida&c={$_SESSION['carteira']}");
        }     
                $read->getResult();
                $agora = 0;
                foreach ($read->getResult() as $value) {
    ...
    public function MenosUm() {
        ...
        if($_SESSION['carteira'] == "geral"){
            $read = new \Source\Models\Read();
                    $read->ExeRead("app_faturas", "WHERE  status = :s AND MONTH(vencimento_em) = :m AND YEAR(vencimento_em) = :y AND modo = :v",
                "s=paid&m={$param1}&y={$param2}&v=saida");
        }else{
        
        $read = new \Source\Models\Read();
        $read->ExeRead("app_faturas", "WHERE  status = :s AND MONTH(vencimento_em) = :m AND YEAR(vencimento_em) = :y AND modo = :v AND carteira_id = :c",
                "s=paid&m={$param1}&y={$param2}&v=saida&c={$_SESSION['carteira']}");
        }   
                $read->getResult();
                $agora = 0;
                foreach ($read->getResult() as $value) {
    ...
    public function buscar() {
        ...
        if (!empty($filtro["buscar"])) {
            ...
            $read = new \Source\Models\Read();
            $read->ExeRead("app_faturas", "WHERE  modo = :a {$prepPeriodo} {$prepCarteira} {$prepCategoria} {$prepStatus} LIMIT :limit OFFSET :offset",
                    "a=saida{$paramPeriodo}{$paramCarteira}{$paramCategoria}{$paramStatus}&limit={$pager->getLimit()}&offset={$pager->getOffset()}");
      ...
      "a=saida&m={$m}&y={$y}&limit={$pager->getLimit()}&offset={$pager->getOffset()}");
           return $read->getResult();
        }
