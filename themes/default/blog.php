    ...
    <div class="row">
        <div class="col-lg-11 mx-auto" id="blog">
            ...
            <div class="row py-5">
                ...
                $resultado->ExeRead("app_post", "WHERE categoria = :a LIMIT :limit OFFSET :offset",
                        "a=blog&limit={$pager->getLimit()}&offset={$pager->getOffset()}"
                        );
                $resultado->getResult();
                foreach ($resultado->getResult() as $value) {
                    ...
                    <div class="col-lg-4">
                        <figure class="rounded p-3 bg-white shadow-sm">
                            <img src="<?= CONF_URL_BASE ?>/admin/uploads/<?= $value["imagem"] ?>" alt="<?= $value["title"] ?>" title="<?= $value["title"] ?>" class="w-100 card-img-top">
                            <figcaption class="p-4 card-img-bottom">
                                ...
                                <button class="btn btn-info"><a href="<?= CONF_URL_BASE ?>/<?= $value["slug"] ?>" title="<?= $value["pagina"] ?>" style="text-decoration: none; color:#fff;"> <?= $value["pagina"] ?> ... saiba mais </a> </button>
